...
# Major Leagues/Competitions to focus on (IDs may vary by API)
MAJOR_COMPETITIONS = [
        ...
        for stage in stages:
            ...
            for event in events:
                match = {
                    ...
                    'country': country,
                    'home_team': event.get('T1', [{}])[0].get('Nm', 'Unknown') if event.get('T1') else 'Unknown',
                    'away_team': event.get('T2', [{}])[0].get('Nm', 'Unknown') if event.get('T2') else 'Unknown',
                    ...
                    'home_position': event.get('T1', [{}])[0].get('Rnk', None) if event.get('T1') else None,
                    'away_position': event.get('T2', [{}])[0].get('Rnk', None) if event.get('T2') else None,
                }
...
class ContentGenerator:
    ...
    def generate_prediction(match: Dict) -> str:
        ...
        # Simple prediction logic (can be enhanced with actual stats)
...
"""
    ...
    def generate_telegram_live_score(match: Dict) -> str:
        ...
        minute_display = f" ({match['minute']}')" if match['is_live'] and match['minute'] else ""
...
"""
        ...
        for comp, comp_matches in by_competition.items():
            ...
            for match in comp_matches:
                ...
                message += f"⏰ {time_str} | {match['home_team']} vs {match['away_team']}\n"
...
""".strip()
    ...
    def generate_facebook_daily_teaser(matches: List[Dict]) -> str:
        ...
        for match in major_matches:
            match_preview += f"⚽ {match['home_team']} vs {match['away_team']}\n"
...
class FootballBot:
    ...
    def run(self):
        ...
        logger.info(f"🔴 Found {len(live_matches) if live_matches else 0} live matches")
        
        # === POSTING LOGIC ===
        
        # 1. Morning summary (8 AM GMT)
        if current_hour == 8:
            self._post_daily_summary(major_matches if major_matches else matches[:10])
    ...
    def _post_upcoming_match(self, match: Dict):
        ...
        logger.info(f"📢 Posted upcoming match: {match['home_team']} vs {match['away_team']}")
    ...
    def _post_live_score(self, match: Dict):
        ...
        logger.info(f"🔴 Posted live score: {match['home_team']} {match['home_score']}-{match['away_score']} {match['away_team']}")
    
    def _post_finished_match(self, match: Dict):
        """Post finished match result"""
        match_id = f"finished-{match.get('id', f'{match['home_team']}-{match['away_team']}')}"
        ...
        logger.info(f"✅ Posted finished match: {match['home_team']} {match['home_score']}-{match['away_score']} {match['away_team']}")
